...
const findReposWithActions = defineCachedFunction(async () => {
    ...
    const reposActionsPromises = reposWithActions.map(async (repo) => {
        const repoActions = await listActionRuns({ path: { owner: repo.owner?.login!, repo: repo.name! } })

        if (repoActions.error) {
            console.error(`Error ${repoActions.response!.status} (${repo.full_name}): ${repoActions.error.message}`);
            return {
                repo: repo,
                actions: {
                    total_count: 0,
