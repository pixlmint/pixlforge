        ...
        <div class="feed-item-content">
            <template v-if="entry.type === 'create_issue' || entry.type === 'comment_issue'">
                <div class="feed-item-content-header">
                    <div>Issue</div>
                    <div> - </div>
                    <div><nuxt-link :to="`/repos/${entry.repo}`">{{ entry.repo }}</nuxt-link></div>
                </div>
                <div>{{ entry.content!.text }}</div>
                <div class="feed-item-content-footer">
                    #{{ entry.content!.issueId }}
                </div>
            </template>
            <!-- XXX: Should commits even show up here? -->
            <template
                v-else-if="(entry.type === 'commit_repo' || entry.type === 'mirror_sync_push') && entry.content !== undefined && entry.content.HeadCommit !== undefined && entry.content.HeadCommit !== null">
                <div class="feed-item-content-header">
                    <div>Commit</div>
                    <div>-</div>
                    <div><nuxt-link :to="`/repos/${entry.repo}`">{{ entry.repo }}</nuxt-link></div>
